...
public class ESourceFile {
	...
	public static class EspLocation {
		...
		int length;
		EspPart part;
		public EspLocation(int offset, EspPart part) { this.offset = offset; this.part = part; this.length = this.part.length(); }
	...
	public static class JavaSource implements Comparable<JavaSource> {
		...
		public JavaSource(EspPart part) { this(part.getText(), part); }
	...
	private TreeMap<String, JavaSource> staticMethods = new TreeMap<String, JavaSource>();
	...
	private TreeMap<String, String> externals = new TreeMap<String, String>();
	...
	public void addConstructor(JavaSource source) {
		addConstructor(constructors.size(), source);
	}

	public void addConstructor(int index, String source) {
		addConstructor(index, new JavaSource(source));
	}
	
	public void addConstructor(String source) {
		addConstructor(new JavaSource(source));
	}
	
	public void addAsset(String canonicalName, String source) {
		externals.put(canonicalName, source);
	}
	...
	public void addImport(JavaSource source) {
		...
		if(src.startsWith("static ")) {
			source.source = src.substring(7);
			staticImports.put(src, source);
		} else {
	...
	public void addImport(String source) {
		addImport(new JavaSource(source));
	}
	...
	public void addMethod(String name, JavaSource source) {
		checkSource();
		if(source.source.contains(" static ")) {
			staticMethods.put(name, source);
		} else {
	...
	public void addMethod(String name, String source) {
		addMethod(name, new JavaSource(source));
	}
	
	public void addMethod(String name, String source, List<EspLocation> locations) {
		addMethod(name, new JavaSource(source, locations));
	}
	...
	public void finalizeSource() {
		...
		sb.append(" {\n");
		for(JavaSource method : staticMethods.values()) {
			sb.append('\n');
	...
	public String[] getAssets() {
		return externals.keySet().toArray(new String[externals.size()]);
	}
