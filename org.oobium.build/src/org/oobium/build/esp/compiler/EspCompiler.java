...
public class EspCompiler {
	...
	public static EspCompiler newEspCompiler(String packageName) {
		return new EspCompiler(packageName);
	}
...
//					sb.append("f.appendChild(s);");
	...
	private void buildInitializer(AssetElement element) {
		if(element.hasAsset()) {
			...
			if(containers.size() > 0) {
				prepForMarkup(body);
				for(EspPart embedded : containers) {
					body.append(getCodeVar(code, embedded)).append(" = ");
					build(embedded, body);
					body.append(';');
				}
			}
	...
	void buildMethodSignature(MethodSignature method) {
		...
		if(method.isConstructor()) {
			...
			ParametricRuleset rule = (ParametricRuleset) method;
			...
			if(declaration.hasProperties()) {
				...
				styleCompiler.buildStyleProperties(declaration, sb);
				sb.append("\");\n");
...
//			resolver.add(dom);
	...
	private ESourceFile compileEss() {
		...
		for(int i = ix; i < parts.size(); i++) {
			...
			if(part.isA(StyleElement)) {
				buildInitializer((StyleElement) part);
			}
		}
		...
		String asset = pkg.replace('.','/') + "/" + dom.getSimpleName() + ".css";
		body = new StringBuilder();
		body.append("/**\n");
		body.append(" * Auto-generated from ").append(pkg).append('.').append(dom.getSimpleName()).append(".ess\n");
		body.append(" * ").append(httpDate(System.currentTimeMillis())).append("\n");
		body.append(" */\n");
		enableEscaping = false;
		for( ; ix < parts.size(); ix++) {
	...
	String getCodeVar(AssetPart part, EspPart var) {
		String name = varName(part.getDom().getSimpleName().replace('.', '_'), false);
