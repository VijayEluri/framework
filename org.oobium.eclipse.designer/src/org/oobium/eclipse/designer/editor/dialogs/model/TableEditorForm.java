...
public abstract class TableEditorForm extends Composite {
	...
	protected final ModelDialog dlg;
	...
	public TableEditorForm(ModelDialog dlg, Composite parent) {
	...
	protected TableItem addItem(String...text) {
		TableItem item = new TableItem(table, SWT.NONE);
		item.setText(text);
		return item;
	}
		
	protected void setReadOnly(String name, boolean readonly) {
		if(readonly) {
			this.readonly.add(name);
		} else {
			this.readonly.remove(name);
		}
		for(TableItem item : table.getItems()) {
			if(name.equals(item.getText(0))) {
				setItemUI(item);
			}
		}
	}
