...
public class DynModels {
	...
	private static final DynModels instance = new DynModels();
	
	public static DynModel getClass(String packageName, String simpleName) {
		return getClass(packageName + "." + simpleName);
	}
	
	public static DynModel getClass(String fullName) {
	...
	Model newInstance(String fullName) throws InstantiationException, IllegalAccessException, ClassNotFoundException {
		compile();
		Model model = (Model) fileManager.getClassLoader(null).loadClass(fullName).newInstance();
		return model;
	}
